...
class OPAClient:
    ...
    def __init__(self, opa_url: str = "http://localhost:8181", policy_path: str = "authz/allow"):
        """
    ...
    async def upload_policy(self, policy_id: str, policy_content: str) -> bool:
        ...
        try:
            async with self.session.put(url, data=policy_content, headers={"Content-Type": "text/plain"}) as response:
...
class PolicyEngine:
    ...
    def __init__(self, opa_url: str = "http://localhost:8181", enable_caching: bool = True):
        """
...
"""
    ...
    async def register_policy(self, policy: PolicyDocument, upload_to_opa: bool = True) -> bool:
        """
        ...
        if upload_to_opa:
            success = await self.opa_client.upload_policy(policy.policy_id, policy.content)
            if not success:
    ...
    async def evaluate(self, policy_id: str, input_data: dict) -> PolicyDecision:
        ...
        # Query OPA
        decision = await self.opa_client.query(input_data, policy_path=f"policies.{policy_id}")
    ...
    async def evaluate_batch(self, evaluations: list[tuple[str, dict]]) -> list[PolicyDecision]:
        ...
        """
        tasks = [self.evaluate(policy_id, input_data) for policy_id, input_data in evaluations]
