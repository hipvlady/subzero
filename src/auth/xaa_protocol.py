...
class XAAProtocol:
    ...
    async def delegate_token(
        ...
        delegation_chain = self._get_or_create_chain(
            original_token_id,
            original_subject,
            target_subject
        )
        ...
        print(f"🔗 Token delegated: {original_subject} → {target_subject} (depth: {delegation_chain.depth}, {latency_ms:.2f}ms)")
    ...
    def get_metrics(self) -> Dict:
        ...
        active_delegations = sum(
            ...
            if token.token_type == XAATokenType.DELEGATED
        )
