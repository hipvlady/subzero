...
class OAuth2PKCEServer:
    ...
    async def handle_authorization_callback(
        ...
        try:
            ...
            if authorization_code not in self.authorization_codes:
                ...
                return await self._exchange_code_with_auth0(
                    authorization_code,
                    client_id,
                    redirect_uri,
                    code_verifier
            ...
            if not self.pkce_validator.verify_code_challenge(
                code_verifier,
    ...
    async def _exchange_code_with_auth0(
        self,
        code: str,
        client_id: str,
        redirect_uri: str,
    ...
    async def introspect_token(self, token: str) -> Dict:
        ...
        try:
            ...
            verified_claims = jwt.decode(
                token,
                self.client_secret or 'default-secret',
                algorithms=['HS256'],
    ...
    async def get_metrics(self) -> Dict:
        ...
        return {
            ...
            'active_authorization_codes': len([
                c for c in self.authorization_codes.values()
                if not c.used and time.time() < c.expires_at
            ]),
            'active_refresh_tokens': len([
                t for t in self.refresh_tokens.values()
                if not t['used'] and time.time() < t['expires_at']
            ]),
