    ...
    async def query_events(
        ...
        if actor_id and actor_id in self.by_actor:
            actor_indices = set(
                ...
                if e.event_id in self.by_actor[actor_id]
            )
        ...
        if event_type and event_type in self.by_type:
            type_indices = set(
                i for i, e in enumerate(self.events)
                if e.event_id in self.by_type[event_type]
            )
...
class ComplianceManager:
    ...
    def _check_gdpr_compliance(self, events: List[AuditEvent]) -> Dict:
        ...
        all_encrypted = all(
            isinstance(e.metadata.get('encrypted'), (str, bytes))
            for e in pii_events
        )
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'event_types': {
                ...
                for event_type, event_ids in self.storage.by_type.items()
            },
