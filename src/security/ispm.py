...
class ISPMEngine:
            ...
            try:
                ...
                if result:  # Finding detected
                    finding = SecurityFinding(
                        ...
                        remediation_action=RemediationAction.RESTRICT if rule.auto_remediate else RemediationAction.ALERT
                    )
    ...
    async def check_dormant_agent(self, agent_id: str) -> Optional[Dict]:
        ...
        if not activities:
            return {
                'description': f"Agent {agent_id} has no recent activity",
                'evidence': {'last_activity': None}
            }
    ...
    async def _build_behavioral_baseline(self, agent_id: str):
        ...
        common_resources = [
            r for r, count in resource_counts.items()
            if count >= len(resources) * 0.1  # >10% of accesses
        ]
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            'agents_by_risk': {
                risk.value: sum(
                    1 for p in self.agent_postures.values()
