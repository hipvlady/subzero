...
from src.security.health_monitor import Auth0HealthMonitor, ServiceStatus, CircuitState
...
class GracefulDegradationService:
    ...
    async def _transition_mode(self, new_mode: DegradedMode):
        ...
        if new_mode != DegradedMode.NORMAL and not self.degradation_started:
            ...
            self.metrics = DegradationMetrics(
                ...
                started_at=self.degradation_started
            )
        ...
        if self.audit_service:
            await self.audit_service.log_event(AuditEvent(
                ...
                metadata={
                    ...
                    'degradation_duration': time.time() - self.degradation_started if self.degradation_started else 0
                }
            ))
    ...
    async def validate_credential_cached(
        ...
        should_use_cache = (
            force_cache or
            self.current_mode in [DegradedMode.PARTIAL, DegradedMode.FULL, DegradedMode.EMERGENCY]
        )
    ...
    async def check_permission_cached(
        ...
        if self.audit_service:
            await self.audit_service.log_event(AuditEvent(
                ...
                event_type=AuditEventType.PERMISSION_GRANTED if cached.allowed else AuditEventType.PERMISSION_DENIED,
    ...
    def get_degradation_status(self) -> Dict:
        ...
        if self.metrics.total_auth_attempts > 0:
            cache_hit_rate_auth = (
                self.metrics.cached_auth_success / self.metrics.total_auth_attempts * 100
            )

        cache_hit_rate_authz = 0
        if self.metrics.total_authz_checks > 0:
            cache_hit_rate_authz = (
                self.metrics.cached_authz_success / self.metrics.total_authz_checks * 100
            )
    ...
    async def cleanup_stale_cache(self):
        ...
        stale_credentials = [
            user_id for user_id, cached in self.credential_cache.items()
            if (current_time - cached.cached_at > self.config['credential_cache_ttl'] or
        ...
        stale_permissions = [
            key for key, cached in self.permission_cache.items()
