...
class SignupFraudDetector:
    ...
    async def detect(
        ...
        recent_signups = sum(
            1 for ts in self.signup_velocity[domain]
...
class AccountTakeoverDetector:
    ...
    def _detect_impossible_travel(self, user_id: str, current_location: str) -> bool:
        ...
        recent_logins = [
            login for login in self.login_patterns[user_id]
            if time.time() - login['timestamp'] < 3600  # Last hour
        ]
    ...
    def _is_new_device(self, user_id: str, device_fingerprint: str) -> bool:
        ...
        known_devices = set(
            login.get('device')
            for login in self.login_patterns[user_id]
            if login.get('device')
        )
...
class MFAAbuseDetector:
    ...
    async def detect(
        ...
        if mfa_method == 'push':
            ...
            recent_pushes = sum(
                1 for ts in self.push_bombing[user_id]
        ...
        recent_attempts = [
            attempt for attempt in self.mfa_attempts[user_id]
...
class HallucinationDetector:
    ...
    async def detect(
        ...
        uncertainty_phrases = [
            ...
            'not sure', 'it seems', 'appears to be'
        ]
